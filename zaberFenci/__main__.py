"""zaberFenci command line interface."""
import sys
import zaberFenci
from argparse import ArgumentParser
from ._compat import *

parser = ArgumentParser(usage="%s -m zaberFenci [options] filename" % sys.executable, description="zaberFenci command line interface.", epilog="If no filename specified, use STDIN instead.")
...
parser.add_argument("-V", '--version', action='version',
                    version="zaberFenci " + zaberFenci.__version__)
parser.add_argument("filename", nargs='?', help="input file")

args = parser.parse_args()

if args.quiet:
    zaberFenci.setLogLevel(60)
if args.pos:
    import zaberFenci.posseg
    posdelim = args.pos
    def cutfunc(sentence, _, HMM=True):
        for w, f in zaberFenci.posseg.cut(sentence, HMM):
            yield w + posdelim + f
else:
    cutfunc = zaberFenci.cut
...
if args.dict:
    zaberFenci.initialize(args.dict)
else:
    zaberFenci.initialize()
if args.user_dict:
    zaberFenci.load_userdict(args.user_dict)
