...
class Tokenizer(object):
    ...
    def initialize(self, dictionary=None):
        ...
        with self.lock:
            ...
            # default dictionary
                ...
                cache_file = "zaberFenci.cache"
            # custom dictionary
            else:
                cache_file = "zaberFenci.u%s.cache" % md5(
    ...
    def tokenize(self, unicode_sentence, mode="default", HMM=True):
        ...
        if not isinstance(unicode_sentence, text_type):
            raise ValueError("zaberFenci: the input parameter should be unicode.")
        start = 0
        if mode == 'default':
...
def enable_parallel(processnum=None):
    ...
    if os.name == 'nt':
        raise NotImplementedError(
            "zaberFenci: parallel mode only supports posix system")
    else:
