...
class POSTokenizer(object):
    ...
    def __init__(self, tokenizer=None):
        self.tokenizer = tokenizer or zaberFenci.Tokenizer()
...
def cut(sentence, HMM=True):
    ...
    """
    global dt
    if zaberFenci.pool is None:
        for w in dt.cut(sentence, HMM=HMM):
            yield w
    else:
        parts = strdecode(sentence).splitlines(True)
        if HMM:
            result = zaberFenci.pool.map(_lcut_internal, parts)
        else:
            result = zaberFenci.pool.map(_lcut_internal_no_hmm, parts)
        for r in result:
            for w in r:
                yield w
