"""zaber_nlp command line interface."""
import sys
import zaber_nlp
from argparse import ArgumentParser
from ._compat import *

parser = ArgumentParser(usage="%s -m zaber_nlp [options] filename" % sys.executable, description="zaber_nlp command line interface.", epilog="If no filename specified, use STDIN instead.")
...
parser.add_argument("-V", '--version', action='version',
                    version="zaber_nlp " + zaber_nlp.__version__)
parser.add_argument("filename", nargs='?', help="input file")

args = parser.parse_args()

if args.quiet:
    zaber_nlp.setLogLevel(60)
if args.pos:
    import zaber_nlp.posseg
    posdelim = args.pos
    def cutfunc(sentence, _, HMM=True):
        for w, f in zaber_nlp.posseg.cut(sentence, HMM):
            yield w + posdelim + f
else:
    cutfunc = zaber_nlp.cut
...
if args.dict:
    zaber_nlp.initialize(args.dict)
else:
    zaber_nlp.initialize()
if args.user_dict:
    zaber_nlp.load_userdict(args.user_dict)
