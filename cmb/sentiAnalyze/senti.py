...
class Classify():
    ...
    def clf3(self, sentence):
        sentence = sentence.decode('utf-8')[:100].encode('utf-8')
    ...
    def clf(self, sentence):
        ...
        prob = self.predictions.eval(session=self.sess, feed_dict={self.input_x: x_test, self.dropout_keep_prob: 1})
        return float("%.3f" % prob[0, 0])
    ...
    def get_opinion_sentence(self, text):
        ...
        for k, (key, bgin, end) in enumerate(locations):
            ...
            for j in range(len(text2) - 1, -2, -1):
                if j == -1 or text2[j] in fuhao:
                    pre_sent = text2[j + 1:]
                    break
            ...
            while k < len(sent):
                if k == 0:
                    ...
                    rval.append((key, pre_sent + sent[:k + 1]))
                    break
                k += 1
        return [sent.encode('utf-8') for key, sent in rval]
